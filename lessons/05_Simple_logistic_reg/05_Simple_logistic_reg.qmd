...
-   Because the [expected value is a weighted average](https://nwakim.github.io/F24_EPI_525/lessons/05_RVs_Binomial/05_RVs_Binomial.html#/expectation-of-random-variables), we can say: $$\begin{aligned} E(Y|X) & = P(Y=1|X) \cdot 1 + P(Y=1|X) \cdot 0 \\ & = P(Y=1|X) \cdot 1 \\ & = P(Y=1|X) \\ & = \pi(X) \end{aligned}$$
...
- Note that $$g\left(\mu\right)=\beta_0+\beta_1X_1+\ldots+\beta_kX_k$$ is the general form, but $g\left(\mu\right)$ just represents the transformation we make to mean $\mu$

 

- It's basically like saying $g\left(\mu\right) = \text{logit} (\mu)$ and thus $$ \text{logit} (\mu)=\beta_0+\beta_1X_1+\ldots+\beta_kX_k$$
